...
//        loadSVMModel();
    ...
    class ThreadInstantAuth extends Thread
    {
        ...
        public void run()
        {
            ...
            peakIndex = Peaks.findPeaks(convBuffer,60,0.05);
            aoFineValues.clear();
...
//            progressbar.setProgress(ans);
        ...
        float avgHeartRate = meanHeartRate/(float)heartRatequeue.size();
        float stdHeartRate = 0;
        for (Float index: heartRatequeue) {
        ...
        if (Test_mode == true) {
            ...
            rateline1=rateline;
            heartbeatInfo.setText(String.format("Heart Rate Estimation:%f",meanHeartRate/(float)heartRatequeue.size()));
            heartbeatVar.setText(String.format("Heart Rate Variability:%f",Math.sqrt(stdHeartRate/(float)(heartRatequeue.size()-1))));
        }
        else {
